    ...
    public class OrdersController : Controller
    {
        ...
        public void SelectProductType()
        {
            ...
            foreach (var item in product)
            {
                if(item.ProductID == 1)
                {
                    pro.Add(new SelectListItem { Text = "LPG(Kg)", Value = item.ProductID.ToString() });
                }
                else
                {
                    pro.Add(new SelectListItem { Text = "Condensate(Ltrs)", Value = item.ProductID.ToString() });
                }
        ...
        public ActionResult DriverNameCNIC(string vehiId)
        {
            ...
            if (!string.IsNullOrEmpty(vehiId))
            {
                ...
                if(vehicle.Any())
                {
                    ...
                    {
                        nameCNIC.Add(new ListItemVehicleChange { DrvName = item.Driver.DriverName, DrvCnic = item.Driver.CNIC, PrdType = item.Trailers.FirstOrDefault().LoadingType });
                    }
                }
                else
                {
                    nameCNIC.Add(new ListItemVehicleChange { DrvName = string.Empty, DrvCnic = string.Empty, PrdType = false });
                }

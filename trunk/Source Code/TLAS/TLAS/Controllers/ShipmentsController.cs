...
namespace TLAS.Controllers
{
    ...
    public class ShipmentsController : Controller
    {
        ...
        public ActionResult ActiveShipments(string sortOrder, string currentFilter, string searchString, int? page)
        {
            ...
            ViewBag.ShipCrDateSortParm = sortOrder == "ShipCrDate" ? "ShipCrDate_desc" : "ShipCrDate";
            ViewBag.ShipMoDateSortParm = sortOrder == "ShipMoDate" ? "ShipMoDate_desc" : "ShipMoDate";
        ...
        public ActionResult compartmentPlanning(string vehicleId, int id)
        {
            ...
            if(shipComp.Count() != 0)
            {
                ...
                {
                    list.Add(new ListItem() { CompIdLI = item.ID.ToString(), CompCodeLI = item.CompartmentCode.ToString(), CapacityLI = item.Capacity.ToString(), OrderedQtyLI = item.OrderedQuantity.ToString(), PlannedQtyLI = item.PlannedQuantity.ToString(), ActualQtyLI = item.ActualBCUQuantity.ToString(), ManualQtyLI = item.ManualQuantity.ToString(), isCreatedLI = true });
                }
        ...
        public ActionResult Create([Bind(Include = "ID,IsManual,AccessCardID,DeletedFlag,ShipmentStatusID,OrderID,BayID,BayName,CustomerName,VehicleCode,DriverName,DriverCNIC,CarrierName,ProductID")] Shipment shipment, FormCollection formCollection)
        {
            ...
            if (compCountTempDataUpdate != 0)
            {
                for (int i = 0; i < compCountTempDataUpdate; i++)
                {
                    ...
                    {
                        int compManualQty;
                        if (Int32.TryParse(formCollection["compManualQty" + i], out compManualQty))
                        {
                            shipmentComp.ManualQuantity = compManualQty;
                        }
                        else
                        {
                            shipmentComp.ManualQuantity = null;
                        }
                    }
